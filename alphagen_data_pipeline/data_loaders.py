...
def aggregate_to_am_pm(
    ...
    Args:
        universe_filter: Optional list of ticker symbols to filter. If None, processes all symbols.
    """
    ...
    all_symbols = [d for d in os.listdir(data_path) if os.path.isdir(os.path.join(data_path, d))]
    if not all_symbols:
        print(f"Error: No symbol directories found in '{data_path}'")
        return pd.DataFrame()

    # Apply universe filter if provided (case-insensitive)
    if universe_filter:
        universe_upper = [u.upper() for u in universe_filter]
        symbols = [s for s in all_symbols if s.upper() in universe_upper]
        print(f"Found {len(all_symbols)} symbols in data directory, filtered to {len(symbols)} in universe.")
        if not symbols:
            print(f"Warning: None of the universe stocks found in data directory!")
            print(f"Universe stocks: {universe_filter[:10]}...")
            return pd.DataFrame()
    else:
        symbols = all_symbols
        print(f"Found {len(symbols)} symbols (no universe filter applied).")
